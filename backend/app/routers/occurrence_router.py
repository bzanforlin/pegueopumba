...
router = APIRouter(prefix="/occurrences", tags=["Occurences"])
...
def create_occurrence(request: CreateOccurenceRequest):
    ...
    result = supabase_client.rpc(
        "find_users_within_10km",
        {
            ...
            "lng": str(request.location.longitude)
        }
    ).execute()
    ...
    if affected_user_ids:
        ...
        phone_numbers = [u["phone_number"] for u in users_result.data if u.get("phone_number")]
